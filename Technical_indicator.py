...
for i in tickers:
    ...
    ax0.plot(
        all_data[all_data.symbol == i].loc[start:end, ["Close", "SMA_5", "SMA_15"]]
    )
...
ax1 = plt.subplot2grid((6, 4), (5, 0), rowspan=1, colspan=4, sharex=ax0)
ax1.plot(
    all_data[all_data.symbol == "SBIN.NS"].loc[start:end, ["SMA_ratio"]], color="blue"
)
ax1.legend(["SMA_Ratio"], ncol=3, loc="upper left", fontsize=12)
ax1.set_facecolor("silver")
plt.subplots_adjust(left=0.09, bottom=0.09, right=1, top=0.95, wspace=0.20, hspace=0)
plt.show()
...
ax0.legend(["Close", "SMA_5", "SMA_15"], ncol=3, loc="upper left", fontsize=15)
ax0.plot(
    all_data[all_data.symbol == "SBIN.NS"].loc[start:end, ["Close"]],
)
...
single_stock_df["max_local"] = single_stock_df.iloc[
    argrelextrema(single_stock_df["15Ewm"].values, np.greater)[0]
]["15Ewm"]
# all_data['min_local']=all_data.iloc[argrelextrema(all_data.Close.values,np.less)[0]]['Close']
single_stock_df["min_local"] = single_stock_df.iloc[
    argrelextrema(single_stock_df["15Ewm"].values, np.less)[0]
]["15Ewm"]
...
ax0.legend(["Close", "SMA_5", "SMA_15"], ncol=3, loc="upper left", fontsize=15)
ax0.plot(
    single_stock_df.loc[start:end, ["Close"]],
)
#     ax0.plot(tmp_nparray)
